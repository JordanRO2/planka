...
module.exports = {
  ...
  async fn(inputs) {
    ...
    // Check project-level permissions for editing cards
    const canEditCards = await sails.helpers.users.canEditCards(currentUser.id, project.id);

    const availableInputKeys = ['id', 'isSubscribed'];
    if (canEditCards && boardMembership.role === BoardMembership.Roles.EDITOR) {
